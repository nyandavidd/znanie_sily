...
from SILARAG.rag.rag_inference import EMBEDDING_MODEL, LOG_DB_NAME, EMBEDDING_MODEL, LLM_MODEL
from SILARAG.rag.rag_inference import prompt_denis, \
    setup_database, setup_log_database, retrieve_context, call_model, ask_question, ask_question_creative

from bot import database_connection, embedding_model, conversation_history, log_database_connection, chat_session_id
from bot import bot
...
async def file_handler(message: Message):
    ...
    try:
        ...
        save_directory = 'SILARAG/rag/data'
...
async def command_start_handler(message: Message, state: FSMContext) -> None:
    ...
    """
    await state.clear()
    await message.answer(f"Привет, {html.bold(message.from_user.full_name)}! Добро пожаловать в нашего бота. Нажмите '🔐 Авторизация', чтобы продолжить.\nДля гостевого доступа используйте логин guest и пароль guest.", reply_markup=auth_kb())
...
async def answer(message: Message, state: FSMContext):
    ...
    response, context, meta_datas, sources = ask_question(message.text, database_connection, embedding_model, conversation_history, log_database_connection, chat_session_id)
...
async def creative_answer(message: Message, state: FSMContext):
    ...
    response, context, meta_datas, sources = ask_question_creative(message.text, database_connection, embedding_model, conversation_history, log_database_connection, chat_session_id)
    fw = ', '.join(set(meta_datas))
    ss = ', '.join(set(sources))
    ans = f"""
    \nОтвет:
